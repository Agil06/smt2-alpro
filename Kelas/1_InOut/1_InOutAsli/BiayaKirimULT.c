...
void formatPrice(char *dest, float price) {
    char buffer[50];
    sprintf(buffer, "%.2f", price);
    int len = strlen(buffer);
    int dotPos = len - 3; // position of the dot
    int commaCount = (dotPos - 1) / 3;
...
int main()
{
  ...
  formatPrice(formattedPrice, totalBiaya);
  printf("Total biaya adalah (%.2f * 5000 + %.2f * 2000) = %s rupiah.\n", berat, jarak, formattedPrice);
